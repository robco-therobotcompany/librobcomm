 ...
 *
 * This example demonstrates reading joint data (joint angle and torque) during a movement. Note
 * that this requires read-write access to the robot.
 *
 */
	...
	while(true) {
		...
		if(read_counter++ >= 1000) {
			...
			for(int i = 0; i < joints.size(); i++) {
				std::cout << "  " << i + 1 << " ( " << joints[i].q() << ", " << joints[i].torque()
				          << " )" << std::endl;
			}
		}
